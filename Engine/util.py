...
from Engine.terms import TokenSpan

CSV_DELIMETER = ';'


TAG_1 = {0: 'O', 1: 'I'}
TAG_2 = {0: 'O', 1: 'P', 2: 'N', 3: 'M'}

def get_tag_set_size(type=0):
    if type == 0:
        return len(TAG_1)
    elif type == 1:
        return len(TAG_2)
...
def check_version(config, model_class, model_path):
    if os.path.exists(model_path):
        ...
        # version check
        loaded_version = config_dict.get('symux_version', '1.0')
        if 'rel_classifier.weight' in state_dict and loaded_version != model_class.VERSION:
            msg = ("Current Engine version (%s) does not match the version of the loaded model (%s). "
